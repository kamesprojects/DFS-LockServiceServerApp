...
$JAR = "target\extentservice-server.jar"
$LOGDIR = "logs"
...
switch ($command) {
  "start" {
    if (-not $arg1 -or -not $extentRoot) {
      throw "Pouzitie: .\extentservice.ps1 start <port> <extentRootPath>"
    }
    ...
    New-Item -ItemType Directory -Force -Path $extentRoot | Out-Null
    ...
    Write-Host "[INFO] Spustam $SERVICE na porte $port (root '$extentRoot')..."
    $p = Start-Process -NoNewWindow -FilePath "java" `
          -ArgumentList @("-jar", $JAR, "--port", $port, "--root", $extentRoot) `
  ...
  "stop" {
    if (-not $arg1) { throw "Pouzitie: .\extentservice.ps1 stop <address|port>" }
    $port = Get-PortFromArg $arg1
    $pidFile = "$LOGDIR\$SERVICE-$port.pid"
    if (Test-Path $pidFile) {
